	...
	for i := 1; i < len(c.Args()); i++ {
		...
		err = filepath.Walk(fromdir, func(path string, info os.FileInfo, err error) error {
			...
			if !info.IsDir() && (filepath.Ext(tofile) == ".flac" || filepath.Ext(tofile) == ".m4a") {
				...
				cmd := exec.Command("ffmpeg", "-i", path, "-ab", "256k", "-map_metadata", "0", "-id3v2_version", "3", tofile)
				err = cmd.Run()
				if err != nil {
					log.WithFields(log.Fields{"message": err.Error(), "from": path, "to": tofile}).
						Error("ffmpeg error")
					return err
				} else {
				...
				cmd := exec.Command("rsync", "-Prh", path, tofile)
				err = cmd.Run()
				if err != nil {
					log.WithFields(log.Fields{"message": err.Error(), "from": path, "to": tofile}).
						Error("ffmpeg error")
